...
## 🚨 CARA LOGIN ADMIN YANG BENAR (Setelah Deploy)

### Opsi 1: Pakai Fallback Lokal (Tanpa MongoDB)

Jika MongoDB belum terkoneksi atau belum setup, login admin pakai **fallback lokal**:

- **Username:** `admin`
- **Password:** `admin123`

> ⚠️ Ini hanya untuk testing lokal. Setelah MongoDB jalan, WAJIB ganti password via MongoDB.

### Opsi 2: Setup Admin via MongoDB (Produksi)

Setelah MongoDB terkoneksi, setup admin dengan password kuat:

**Via Postman/Thunder Client:**
- Method: `POST`
- URL: `https://www.alfaofficial.my.id/api/admin-auth?action=setup`
- Body (JSON):
```json
{
  "username": "admin",
  "password": "AlfaHosting2026!Kuat"
}
```

**Via cURL:**
```bash
curl -X POST https://www.alfaofficial.my.id/api/admin-auth?action=setup \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"AlfaHosting2026!Kuat"}'
```

Setelah berhasil, login dengan kredensial tersebut.

---

## 📱 RESPONSIVE DESIGN

Website sudah **fully responsive** untuk semua device:

| Breakpoint | Device | Layout |
|------------|--------|--------|
| > 1024px | Desktop | 4 kolom grid, full navigation |
| 768-1024px | Tablet | 2 kolom grid, hamburger menu |
| 480-768px | Mobile | 1 kolom grid, simplified UI |
| < 480px | Small Mobile | Stack layout, minimal padding |

**Tested on:**
- ✅ Desktop (1920x1080, 1366x768)
- ✅ Tablet (iPad, Android Tablet)
- ✅ Mobile (iPhone, Android)
- ✅ Small Mobile (iPhone SE, Android compact)

---

## 🗄️ MIGRASI DATA: localStorage → MongoDB

### Status Migrasi:

| Data | Status | API Endpoint | Keterangan |
|------|--------|--------------|-----------|
| Products | ✅ Siap | `/api/products` | CRUD lengkap, init default |
| Orders/Sales | ⏳ Partial | `/api/transactions` | User transactions ada, admin view perlu tambahan |
| Testimonials | ✅ Siap | `/api/content?type=reviews` | Approve/reject system |
| Newsletter | ✅ Siap | `/api/content?type=newsletter` | Export CSV, delete |
| Flash Sale | ✅ Siap | `/api/content?type=flash-sale` | Start/stop, history |
| Settings | ✅ Siap | `/api/settings` | Enkripsi AES-256-GCM |
| Vouchers | ⏳ Perlu | `/api/vouchers` | Belum dibuat |
| Cart | ❌ Tetap lokal | - | User-specific, tidak perlu DB |
| Wishlist | ❌ Tetap lokal | - | User preference, tidak kritis |
| Theme | ❌ Tetap lokal | - | UI preference |

### Cara Inisialisasi Produk ke MongoDB:

Setelah admin berhasil login, buka console → ketik:

```javascript
fetch('/api/products?action=init', {
  method: 'POST',
  headers: {'X-Admin-Token': sessionStorage.getItem('admin_token')}
}).then(r => r.json()).then(console.log)
```

Ini akan memasukkan 30 produk default ke MongoDB.

---

*Update Migrasi MongoDB — Senin, 27 April 2026 pukul 08:00 WIB*
